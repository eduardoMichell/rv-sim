...
export function createText(code: string, data: Array<Data>): Text {
    ...
    if (hasSegment(lineArrayCode, '.data') && !hasSegment(lineArrayCode, '.text')) {
        return {
            source: [],
            basic: []
        }
    ...
    } else if (!hasSegment(lineArrayCode, '.data') && hasSegment(lineArrayCode, '.text')) {
...
export function hasSegment(lineArrayCode: string[][], segment: string): boolean {
    return lineArrayCode.some(line => line.some(element => element.includes(segment)));
}

function processPseudoInstructions(source: Array<string[]>, basic: Array<string[]>, data: Array<Data>) {
    const newSource: Array<string[]> = [];
    const newBasic: Array<any> = [];
    for (let i = 0; i < source.length; i++) {
        const { basicWithPseudo } = createPseudoInstruction(basic[i], newBasic, data);
...
function createPseudoInstruction(line: string[], newBasic: Array<string[]>, data: Array<Data>) {
    ...
    switch (instruction) {
        ...
        case 'nop':
            ...
            break;
        case 'la':
            basicWithPseudo = convertLoadAddressWithOnlyLabel(line, newBasic.length);
            break;
        case 'lb':
        case 'lbu':
...
function convertSetGreaterThanZero(line: string[]) {
    return line.length === 3 ? { inst: [['slt', line[1], 'x0', line[2]]], isPseudo: true } : { inst: [line], isPseudo: false };
}

function convertSetEqualToZero(line: string[]) {
    return line.length === 3 ? { inst: [['sltiu', line[1], line[2], '1']], isPseudo: true } : { inst: [line], isPseudo: false };
}


function convertBranchIfGreaterThanOrEqualZero(line: string[]) {
    return line.length === 3 ? { inst: [['bge', line[1], 'x0', line[2]]], isPseudo: true } : { inst: [line], isPseudo: false };
}


function convertBranchIfEqualZero(line: string[]) {
    return line.length === 3 ? { inst: [['beq', line[1], 'x0', line[2]]], isPseudo: true } : { inst: [line], isPseudo: false };
}


function convertBranchIfNotEqualZero(line: string[]) {
    return line.length === 3 ? { inst: [['bne', line[1], 'x0', line[2]]], isPseudo: true } : { inst: [line], isPseudo: false };
}

function convertBranchIfLessThanZero(line: string[]) {
    return line.length === 3 ? { inst: [['blt', line[1], 'x0', line[2]]], isPseudo: true } : { inst: [line], isPseudo: false };
}

function convertBranchIfLessThanOrEqualZero(line: string[]) {
    return line.length === 3 ? { inst: [['bge', 'x0', line[1], line[2]]], isPseudo: true } : { inst: [line], isPseudo: false };
}

function convertBranchIfGreaterThanZero(line: string[]) {
    return line.length === 3 ? { inst: [['blt', 'x0', line[1], line[2]]], isPseudo: true } : { inst: [line], isPseudo: false };
}

function convertSetNotEqualToZero(line: string[]) {
    return line.length === 3 ? { inst: [['sltu', line[1], 'x0', line[2]]], isPseudo: true } : { inst: [line], isPseudo: false };
}


function convertSetLessThanZero(line: string[]) {
    return line.length === 3 ? { inst: [['slt', line[1], line[2], 'x0']], isPseudo: true } : { inst: [line], isPseudo: false };
}
...
function convertMove(line: string[]) {
    return line.length === 3 ? { inst: [['add', line[1], 'x0', line[2]]], isPseudo: true } : { inst: [line], isPseudo: false };
}

function convertNot(line: string[]) {
    return line.length === 3 ? { inst: [['xori', line[1], line[2], '-1']], isPseudo: true } : { inst: [line], isPseudo: false };
}
...
function convertJumpRegister(line: string[]) {
    return line.length === 2 ? { inst: [['jalr', 'x0', line[1], '0']], isPseudo: true } : { inst: [line], isPseudo: false };
}

function convertLoadImmediate(line: string[]) {
    return line.length === 3 ? { inst: [['addi', line[1], 'x0', line[2]]], isPseudo: true } : { inst: [line], isPseudo: false };
}

function convertLoadWord(line: string[], numberOfInstructions: number, data: Array<Data>) {
    if (line.length === 3) {
        const t1 = line[1] || '';
        const t2 = line[2] || '';
        if(isNaN(Number(t2)) && !t2.includes("(") &&  !t2.includes(")")){
            ...
            const newBasicWithPseudo: Array<string[]> = [
                ['auipc', t1, '64528'],
                ['lw', t1, `${(labelData?.memoryPosition || 0) - (pcAddress + auipcImm)}(${t1})`]
            ];
           ...
           if(Number(t2) <  -2048 || Number(t2) > 2047 ){
            ...
            const t1 = line[1];
            const t2 = line[2].includes('(') ? `${line[2].slice(0, line[2].indexOf('('))}(${line[2].slice(line[2].indexOf('(') + 1, line[2].length - 1)})` : `0(${line[2]})`;
...
function convertLoadAddressWithOnlyLabel(line: string[], numberOfInstructions: number) {
    if (line.length === 3) {
        const label = line[1] || '';
        const newBasicWithPseudo: Array<string[]> = [
            ['auipc', label, '64528'],
            ['addi', line[1], line[1], ((numberOfInstructions) * -4).toString()]
        ];
        return { inst: newBasicWithPseudo, isPseudo: true };
    } else {
        return { inst: [line], isPseudo: false };
...
function convertBranchIfGreater(line: string[]) {
    ...
    return { inst: [[instruction, line[2], line[1], line[3] || '']], isPseudo: true };
}
...
function separateCode(code: string): Array<string[]> {
    ...
    const separatedLineCode = codeWithoutLineSpace.split('\n').filter(line => line.trim() !== '');
...
function removeComments(code: Array<string[]>): Array<string[]> {
    return code
        .map(line => line.map(element => element.replace(/\t/g, '')).filter(element => !element.includes('#')))
...
function createTextBasic(source: string[][]): string[][] {
    ...
    for (let i = 0; i < basic.length; i++) {
        for (let j = 0; j < basic[i].length; j++) {
            ...
            if (exist) {
                element = registerKeys.reduce((acc, key) => {
                    return element.includes(key) ? acc.replace(key, `x${RegFile[key].value}`) : acc;
                }, element);
                basic[i][j] = element;
            }
        }
...
function createLineArrayCode(separatedLineCode: string[]): Array<string[]> {
    ...
    for (const line of separatedLineCode) {
        const words = line.split(/("[^"]*"\s*|\s+)/).filter(word => word.trim() !== '');
