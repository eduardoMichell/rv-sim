...
export function convertConfigToText(code: any, pc: number) {
  ...
  data.address.forEach((address: string, i: number) => {
    text += `${address}     ${data.code[i]}        ${
      data.basic[i].inst
    }${' '.repeat(24 - data.basic[i].inst.toString().length)}${i + 1}     ${
      data.source[i]
    }\n`;
  });
...
function checkDataElement(element: Data) {
  ...
  switch (element.directive) {
    ...
    default:
      const message =
        element.directive.length > 0
          ? `"${element.directive}" is not a valid directive`
...
function validateLabel(label: string) {
  ...
  if (/^[0-9]/.test(labelWithoutColon)) {
    return {
      error: true,
      message: `"${labelWithoutColon}" cannot start with a number`,
    };
  }
  ...
  if (!regex.test(labelWithoutColon)) {
    return {
      error: true,
      message: `"${labelWithoutColon}" cannot have special characters`,
    };
  }
...
function checkDuplicateLabel(data: Data[]) {
  const duplicated = data.find(
    (item, index) => data.findIndex((el) => el.label === item.label) !== index
  );
  if (duplicated) {
  ...
  if (containsString(line.basic)) {
    if (containsDifferentDirectives(line.basic, ['.ascii', '.string'])) {
      return {
        error: true,
        message:
          'It is not possible to define more than one directive in a label',
      };
    }
    ...
    const basicWithoutQuote = elementWithoutDirectives
      .map((element) => element.replace(/"/g, ''))
      .filter(Boolean);
    line.basic = convertStringToNumberArray(
      basicWithoutQuote.join(' ').replace(/\.string/g, '.ascii')
    );
  } else {
...
function areAllIntegers(array: string[]) {
  for (const item of array) {
    if (!/^-?\d+$/.test(item)) {
      return {
        error: true,
        message: `"${item}" is not a valid integer number`,
      };
    }
  }
  return { error: false, message: '' };
...
function containsDifferentDirectives(
  array: string[],
  directivesToIgnore: string[]
) {
  const ignoreSet = new Set(directivesToIgnore);
  const directivesAux = Directives.filter(
    (element: string) => !ignoreSet.has(element)
  );
  return array.some((element) =>
    directivesAux.some((directive: string) => element.includes(directive))
  );
}
...
function checkRFormatInst(line: string[]) {
  ...
  if (!RegFile[t1] || !RegFile[t2] || !RegFile[t3]) {
    return {
      error: true,
      message: `"${t1}" or "${t2}" or "${t3}" operand is of incorrect type`,
    };
  }
...
function checkIFormatInst(line: string[]) {
  ...
  if (!RegFile[t1] || !RegFile[t2]) {
    return {
      error: true,
      message: `"${t1}" or "${t2}" operand is of incorrect type`,
    };
  }
...
function checkUFormatInst(
  line: string[],
  loadAddressLine: string[],
  data: Data[]
) {
